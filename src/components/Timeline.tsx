      ...
      <div className="space-y-8">
        {timelineEvents.map((event, index) => (
          <div key={`${event.year}-${index}`} className="relative flex items-start">
            <Card 
              className={cn(
                "flex-1 cursor-pointer transition-all duration-300 hover:shadow-lg border-border/50",
                selectedEvent === index && "ring-2 ring-primary shadow-xl scale-[1.02]"
              )}
              onClick={() => setSelectedEvent(selectedEvent === index ? null : index)}
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-primary text-xl font-bold">{event.year}</CardTitle>
                  ...
                  {event.image && (
                    <div className="mb-4">
                      <img 
                        ...
                        className="w-full h-48 object-cover rounded-lg shadow-md border border-border/20"
                      />
                    </div>
                  )}
